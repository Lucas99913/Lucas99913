<div align="center">

# Lucas Giorsetti

**Full Stack Developer** · Développeur Full Stack  
Web · Mobile · Product

[Portfolio](https://whoislucas.com) · [LinkedIn](https://www.linkedin.com/in/lucas-giorsetti-795198236/) · [Email](mailto:lucas.giorsetti@gmail.com)

<br />

[🇬🇧 English](#-english) · [🇫🇷 Français](#-français)

<br />

<img src="https://img.shields.io/badge/Epitech-4th%20Year-000000?style=flat-square" alt="Epitech" />
<img src="https://img.shields.io/badge/McGill-Business%20Management-ED1B2F?style=flat-square" alt="McGill" />
<img src="https://img.shields.io/badge/Location-Marseille%20%2F%20Montr%C3%A9al-0A66C2?style=flat-square" alt="Location" />
<img src="https://img.shields.io/badge/Open%20to-Opportunities-2ea44f?style=flat-square" alt="Open to opportunities" />

</div>

---

## Technical Skills / Compétences techniques

| | |
|:--|:--|
| **Languages / Langages** | C · C++ · Python · JavaScript / TypeScript · PHP · Dart · HTML / CSS |
| **Frameworks** | React · Node.js · Symfony · Flutter |
| **Cloud & DB** | Google Cloud Platform · Firebase · Firestore · Docker |
| **Tools / Outils** | Git · GitHub · Jira · Linux |

**Full Stack** — web front-end, back-end & cross-platform mobile / développement web front-end, back-end et mobile multiplateforme.

<p align="left">
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/c/c-original.svg" alt="C" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/cplusplus/cplusplus-original.svg" alt="C++" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="JavaScript" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/typescript/typescript-original.svg" alt="TypeScript" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/php/php-original.svg" alt="PHP" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/dart/dart-original.svg" alt="Dart" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" alt="HTML5" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" alt="CSS3" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/react/react-original.svg" alt="React" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/nodejs/nodejs-original.svg" alt="Node.js" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/flutter/flutter-original.svg" alt="Flutter" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/symfony/symfony-original.svg" alt="Symfony" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/firebase/firebase-plain.svg" alt="Firebase" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/googlecloud/googlecloud-original.svg" alt="GCP" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original.svg" alt="Docker" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="36" height="36"/>
  <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" alt="Linux" width="36" height="36"/>
</p>

---

<a id="-english"></a>

## 🇬🇧 English

### About

4th-year computer science student at **Epitech** (Grande École Program) and Full Stack developer with hands-on experience in web and mobile. Co-founder of **PulseTrack**, a connected sports hardware/software startup (Epitech EIP). Currently completing **McGill University’s Certificate in Business Management** — bridging technical execution with finance, marketing, and organizational strategy.

> Explore my work on my [portfolio](https://whoislucas.com) or connect on [LinkedIn](https://www.linkedin.com/in/lucas-giorsetti-795198236/).

### Experience

| Period | Role | Company |
|:--|:--|:--|
| Apr – Aug 2025 | Front-End Developer (Web & Mobile) | **Livtoo**, Marseille |
| Jul – Oct 2022 | Junior Developer | **Winlink System** |
| Jun 2023 – Jun 2024 | President | **Asso’M** — Epitech student association |

**Livtoo** — Dating startup combining digital matchmaking with physical “Safe Place” venues.
- Built the admin web app (dashboard, user management, moderation, analytics)
- Developed the partners Flutter app (iOS / Android) for venue management
- Integrated Firebase Auth, Firestore, and GCP for auth, data, and storage
- Agile delivery with Jira and GitHub

**Winlink System**
- Contributed to a Symfony management application
- Designed and implemented an admin dashboard
- Built and deployed a full newsletter system from scratch

**Asso’M**
- Organized France’s largest student event (~6,000 attendees)
- Led the team, logistics, partners, and communication strategy

### Entrepreneurial Project

#### PulseTrack — MTB Suspension Telemetry
**Epitech EIP (2025 – 2027)** · Co-founder

Democratizing suspension telemetry for mountain bikers — pro-level data under €300 instead of €15k+ team budgets.

- **Hardware** — ESP32 sensor on fork/shock (position, velocity, cycle count)
- **Software** — Flutter app with real-time Bluetooth, session recording, GPS, and AI-driven setup recommendations
- **Business** — Market research, BMC, financial planning, product roadmap, and user iteration
- **Beta** — Mar–May 2026 with MTB testers (NPS, retention, crash rate)

### Selected Academic Projects

| Project | Description |
|:--|:--|
| **AREA** | Automation platform (IFTTT / Zapier-style) connecting third-party services |
| **R-Type** | Classic arcade game recreated in C++ with full multiplayer networking |
| **RayTracer** | Raytracing rendering engine implemented from scratch in C++ |

### Education

| Years | Program | School |
|:--|:--|:--|
| 2021 – 2027 | Expert Degree in Information Technologies | **Epitech** — Grande École, Marseille |
| Aug 2025 – Aug 2026 | Certificate in Business Management | **McGill University**, Montréal |
| 2019 – 2021 | High School Diploma — STI2D | Lycée du Rempart, Marseille |

### Spoken Languages

| Language | Level |
|:--|:--|
| French | Native |
| English | Bilingual (B2) — full academic year at McGill, Montréal |

### Connect

| | |
|:--|:--|
| Portfolio | [whoislucas.com](https://whoislucas.com) |
| LinkedIn | [lucas-giorsetti](https://www.linkedin.com/in/lucas-giorsetti-795198236/) |
| Email | [lucas.giorsetti@gmail.com](mailto:lucas.giorsetti@gmail.com) |
| Location | Marseille / Aix-en-Provence · Montréal |

<sub>Open to internships and full-stack opportunities — feel free to reach out.</sub>

---

<a id="-français"></a>

## 🇫🇷 Français

### À propos

Étudiant en 4ᵉ année d’informatique à **Epitech** (Programme Grande École) et développeur Full Stack, avec une expérience concrète en développement web et mobile. Co-fondateur de **PulseTrack**, une startup hardware/software dédiée au sport connecté (EIP Epitech). Actuellement en **Certificate in Business Management à McGill University** — pour relier l’exécution technique à la finance, au marketing et à la stratégie organisationnelle.

> Découvrez mon travail sur mon [portfolio](https://whoislucas.com) ou connectez-vous sur [LinkedIn](https://www.linkedin.com/in/lucas-giorsetti-795198236/).

### Expérience

| Période | Poste | Entreprise |
|:--|:--|:--|
| Avr. – Août 2025 | Développeur Front-End (Web & Mobile) | **Livtoo**, Marseille |
| Juil. – Oct. 2022 | Développeur Junior | **Winlink System** |
| Juin 2023 – Juin 2024 | Président | **Asso’M** — association étudiante Epitech |

**Livtoo** — Startup de rencontres alliant matchmaking digital et lieux physiques « Safe Place ».
- Conception et développement de l’application web admin (dashboard, gestion utilisateurs, modération, analytics)
- Développement de l’application mobile partenaires (Flutter, iOS / Android) pour la gestion des lieux
- Intégration Google Cloud Platform (Firebase Auth, Firestore, GCP) pour l’auth, la base de données et le stockage
- Travail en Agile avec Jira et GitHub

**Winlink System**
- Contribution au développement d’une application de gestion en Symfony
- Conception et mise en place d’un tableau de bord administrateur
- Création et déploiement d’un système de newsletter complet

**Asso’M**
- Organisation du plus grand événement étudiant de France (~6 000 participants)
- Pilotage d’équipe, logistique, partenaires et stratégie de communication

### Projet entrepreneurial

#### PulseTrack — Télémétrie de suspension VTT
**EIP Epitech (2025 – 2027)** · Co-fondateur

Démocratiser la télémétrie de suspension pour les riders VTT — des données niveau pro à moins de 300 €, contre plus de 15 k€ pour les équipes professionnelles.

- **Hardware** — Capteur ESP32 sur fourche/amortisseur (position, vitesse, cycles)
- **Software** — App Flutter avec Bluetooth temps réel, enregistrement de sessions, GPS et recommandations de setup par IA
- **Business** — Étude de marché, business model canvas, planification financière, roadmap produit et itérations utilisateurs
- **Beta** — Mars–mai 2026 avec testeurs VTT (NPS, rétention, taux de crash)

### Projets académiques sélectionnés

| Projet | Description |
|:--|:--|
| **AREA** | Plateforme d’automatisation (type IFTTT / Zapier) reliant des services tiers |
| **R-Type** | Recréation du jeu d’arcade R-Type en C++ avec multijoueur réseau |
| **RayTracer** | Moteur de rendu raytracing (technologie RTX) développé from scratch en C++ |

### Formation

| Années | Programme | École |
|:--|:--|:--|
| 2021 – 2027 | Titre d’Expert en Technologies de l’Information | **Epitech** — Grande École, Marseille |
| Août 2025 – Août 2026 | Certificate in Business Management | **McGill University**, Montréal |
| 2019 – 2021 | Baccalauréat STI2D | Lycée du Rempart, Marseille |

### Langues parlées

| Langue | Niveau |
|:--|:--|
| Français | Langue maternelle |
| Anglais | Bilingue (niveau B2) — année universitaire complète à McGill, Montréal |

### Contact

| | |
|:--|:--|
| Portfolio | [whoislucas.com](https://whoislucas.com) |
| LinkedIn | [lucas-giorsetti](https://www.linkedin.com/in/lucas-giorsetti-795198236/) |
| Email | [lucas.giorsetti@gmail.com](mailto:lucas.giorsetti@gmail.com) |
| Localisation | Marseille / Aix-en-Provence · Montréal |

<div align="center">

<br />

<sub>Ouvert aux stages et opportunités full stack — n’hésitez pas à me contacter.</sub>

<br />

[↑ Back to top / Retour en haut](#lucas-giorsetti)

</div>
